...
type OperationalStatus = "normal" | "attention" | "critical" | "no_data";
...
function MetricCard({ title, value, detail, icon, iconClasses }: MetricCardProps) {
  return (
...
function TicketsByServiceChart({ services }: { services: ServiceMetrics[] }) {
...
function HourlyLineChart({ activity }: { activity: HourlyActivity[] }) {
...
function AverageWaitChart({ services }: { services: ServiceMetrics[] }) {
...
function StatusDonut({ distribution }: { distribution: StatusDistribution[] }) {
...
function WindowLoadChart({ windows }: { windows: ServiceWindowMetrics[] }) {
